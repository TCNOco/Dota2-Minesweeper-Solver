...
def solve_board(board, num_mines):
    """Solve a minesweeper board given as a matrix and return the best move.

    board -- a 2D list representing the minesweeper board, where None represents an unknown cell,
             a number represents the count of adjacent mines, and 'M' represents a marked mine.
    num_mines -- the total number of mines on the board.
    """
    height = len(board)
    width = len(board[0])
    game = GridMinesweeperGame(width, height, num_mines=num_mines)
    
    # Set up the game state based on the provided board
    for i in range(height):
        for j in range(width):
            ...
            if board[i][j] == 'M':
                ...
                game.cells[cell] = board[i][j]
                game.yet_to_uncover -= 1
    
    state = u.generate_rules(BoardWrapper(game), num_mines)
    ...
    if mines:
        ...
        return 'sweep', random.choice(safest)